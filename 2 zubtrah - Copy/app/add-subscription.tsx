...
export default function AddSubscriptionScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const existing = subscriptions.find((item) => item.id === editingId);
      if (existing) {
        setName(existing.name);
        setPrice(String(existing.price));
        setBillingCycle(existing.billing_cycle as BillingCycle);
        setRenewalDate(existing.renewal_date);
        const d = new Date(existing.renewal_date + 'T00:00:00');
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        await createSubscription({ ...payload, created_at: new Date().toISOString() });
      }
